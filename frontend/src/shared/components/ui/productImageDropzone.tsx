	...
	const onDrop = useCallback(
		async (acceptedFiles: File[]) => {
			if (acceptedFiles.length > 0) {
				...
				console.log(
					`compressed file size: ${compressedFileSizeMb.toFixed(3)}MB`
				);
				console.log(
					`Total reduction is: ${((originalSizeMb - compressedFileSizeMb) * 1024).toFixed(3)}KB`
				);
	...
	const { getRootProps, getInputProps, isDragActive } = useDropzone({
		...
		onDropRejected: fileRejections => {
			fileRejections.forEach(({ file, errors }) => {
				errors.forEach(e =>
					enqueueSnackbar(`File ${file.name} rejected: ${e.message}`, {
						variant: 'error',
					})
				);
			});
